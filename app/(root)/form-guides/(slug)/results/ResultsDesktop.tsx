...
import { QUERY_DATE_FORMAT, LONG_DISPLAY_DATE_FORMAT } from "@/config/date";
...
import FormResultsHeader from "./components/desktop/FormResultsHeader";
import { getActualRaceStatus } from "../../../../utils/race/getActualRaceStatus";

export const ResultsDesktop = () => {
  const { replace } = useRouter();
  const yesterday = getPunterDayJS().add(-1, "d");
  const searchParams = useSearchParams();
  let queryDateObj = getPunterDayJS(searchParams.get("date"));
  queryDateObj = queryDateObj.isValid() ? queryDateObj : yesterday;
  ...
  return (
    <div className="flex min-w-full gap-5 px-10 max-xl:flex-col max-sm:px-3 max-sm:py-4 sm:py-8">
      <div className="w-full xl:w-9/12">
        <FormResultsHeader
          calendarProps={{ toDate: yesterday.toDate() }}
          queryDay={queryDate}
        ...
        <div className="my-8 rounded-lg bg-white">
          <h2 className="p-4 text-xl font-bold">
            {yesterday.isSame(queryDate, "date")
              ? "Yesterday's racing"
            ...
            <RacingTable
              rows={dayMeetings.map((meeting) => ({
                ...
                races: meeting.node.racesCollection.edges.map((race) => ({
                  ...
                  status: getActualRaceStatus(race.node as any),
