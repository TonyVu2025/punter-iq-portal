...
interface FormResultsHeaderProps {
  queryDay?: Dayjs;
...
export const FormResultsHeader: React.FC<FormResultsHeaderProps> = ({
  queryDay,
  onDayChange,
  calendarProps,
}) => {
  const displayDays = Array.from({ length: 4 }, (_, i) =>
    getPunterDayJS().add(-i - 1, "day"),
  );
      ...
      <div className="flex flex-1 justify-start">
        ...
        <div className="flex flex-nowrap text-nowrap border-l">
          {displayDays.map((day, index) => (
            ...
            >
              {index === 0 ? "Yesterday" : day.format("dddd")}
            </DaySelectButton>
          ))}
...
export default FormResultsHeader;
